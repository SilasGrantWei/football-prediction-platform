...
import { BarChart3, LayoutDashboard, Radio, Trophy } from "lucide-react";
      ...
      <header className="sticky top-0 z-30 border-b border-white/70 bg-white/82 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3 sm:px-6 lg:px-8">
          <Link href="/dashboard" className="group flex min-w-0 items-center gap-3">
            <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg bg-slate-950 text-white shadow-lg shadow-slate-900/15 transition group-hover:bg-field">
              <Trophy size={21} aria-hidden />
            </span>
            <span className="min-w-0">
              <span className="block truncate text-base font-black tracking-tight text-ink">世界杯智能推算</span>
              <span className="block truncate text-xs font-medium text-slate-500">实时比分、阵容、赛果推算</span>
            </span>
          </Link>

          <nav className="relative z-40 flex shrink-0 items-center gap-1 rounded-lg border border-slate-200/80 bg-slate-50/90 p-1 shadow-sm">
            {navItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={clsx(
                    "inline-flex h-10 cursor-pointer select-none items-center gap-2 rounded-md px-3 text-sm font-semibold transition",
                    active
                      ? "bg-white text-field shadow-sm ring-1 ring-slate-200/80"
